...
db_sampler = dict(
    type="GT-AUG",
    enable=False,
    db_info_path="data/Waymo/dbinfos_{data_split}_1sweeps_withvelo.pkl",
    sample_groups=[
        dict(VEHICLE=15),
        dict(PEDESTRIAN=10),
        dict(CYCLIST=10),
...
aug_cfg = dict(
    mode="train",
    global_rot_noise=[-0.78539816, 0.78539816], # AffineAug
    global_scale_noise=[0.95, 1.05], # AffineAug
    db_sampler=db_sampler, # GTAug
    class_names=class_names,
)
...
train_pipeline = [
    ...
    dict(type='AffineAug', cfg=aug_cfg),
    dict(type="GTAug", cfg=aug_cfg),
