...
run:
	...
	@echo $(dirname)
	OMP_NUM_THREADS=60 python -m torch.distributed.torchrun --nproc_per_node=8 tools/train.py $(config) --work_dir work_dirs/$(dirname)/
	OMP_NUM_THREADS=60 python -m torch.distributed.torchrun --nproc_per_node=8 tools/dist_test.py $(config) --work_dir work_dirs/$(dirname)/ --checkpoint work_dirs/$(dirname)/latest.pth
