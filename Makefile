...
run:
	...
	OMP_NUM_THREADS=60 python -m torch.distributed.launch --nproc_per_node=8 tools/train.py $(config) --work_dir work_dirs/$(dirname)/ --resume_from work_dirs/$(dirname)/latest.pth
