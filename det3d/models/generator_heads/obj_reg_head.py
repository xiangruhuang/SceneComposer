...
from ..utils import build_mlp_layer 
...
class ObjRegHead(nn.Module):
    def __init__(
        ...
        super(ObjRegHead, self).__init__()

        self.dataset = dataset

        self.channels = channels
        self.mlp = build_mlp_layer(channels)

        self.crit_reg = RegLoss()

        if not logger:
            logger = logging.getLogger("ObjRegHead")
        self.logger = logger

        # a shared convolution 
        logger.info("Finish ObjRegHead Initialization")
