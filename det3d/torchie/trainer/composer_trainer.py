...
from .hooks import (
    ...
    IterTimerHook,
    LrUpdaterHook,
    GANOptimizerHook as OptimizerHook,
    lr_updater,
)
...
class ComposerTrainer(object):
    ...
    def current_lr(self):
        ...
        gen_lr = [group["lr"] for group in self.optimizer['gen'].param_groups]
        dsc_lr = [group["lr"] for group in self.optimizer['dsc'].param_groups]
        return gen_lr + dsc_lr
    ...
    def batch_processor_inline(self, model, data, train_mode, **kwargs):
        ...
        if train_mode:
            losses = model(example, return_loss=True, name=kwargs['name'])
    ...
    def train_gen(self, data_loader, epoch, **kwargs):
        ...
        self.alter = 'gen'
        optimizer = self.optimizer[self.alter]
        lr_scheduler = self.lr_scheduler[self.alter]
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            outputs = self.batch_processor_inline(
                self.model, data_batch, train_mode=True, name=self.alter, **kwargs
            )
    ...
    def train_dsc(self, data_loader, epoch, **kwargs):
        ...
        while self.epoch < max_epochs:
            for i, flow in enumerate(workflow):
                ...
                for _ in range(epochs):
                    if mode.startswith("train") and self.epoch >= max_epochs:
                        return
                    elif mode == "val":
